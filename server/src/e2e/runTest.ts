...
function printHelp(): void {
    console.log(`Language Server E2E Tester
...
async function main(): Promise<void> {
    try {
        ...
        if (options.verbose) {
            process.env["TACT_TEST_VERBOSE"] = "true"
        }

        if (options.suite) {
            process.env["TACT_TEST_SUITE"] = options.suite
        }

        if (options.test) {
            process.env["TACT_TEST_PATTERN"] = options.test
        }
